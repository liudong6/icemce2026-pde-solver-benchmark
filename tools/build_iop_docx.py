...
REFERENCES = [
    ...
    'Patankar S V 1980 Numerical Heat Transfer and Fluid Flow 1st edn (New York: Hemisphere). ISBN:9780891165224',
    'Hoefler T and Belli R 2015 Scientific benchmarking of parallel computing systems: twelve ways to tell the masses when reporting performance results Proc. International Conference for High Performance Computing, Networking, Storage and Analysis (SC15) 73:1-73:12. doi:10.1145/2807591.2807644',
]
    ...
    for row_index, row in enumerate(rows):
        ...
        for cell, value in zip(cells, row):
            ...
            for paragraph in cell.paragraphs:
                ...
                for run in paragraph.runs:
                    run.font.size = Pt(9)
    if headers == ['Case', 'N', 'Monitored', 'Count-only', 'CG', 'J', 'SA']:
        table.autofit = False
        widths = [1.03, 0.42, 1.47, 1.47, 0.62, 0.62, 0.62]
...
def build() -> None:
    ...
    add_paragraph(doc, 'Arithmetic faces are retained to preserve the specified nodal graph-diffusion benchmark and its historical comparisons, not because they are the preferred interface law for composite heat conduction. Interface conductivity is a standard numerical heat-transfer issue [[26]]. For two half-links of equal length with conductivities a and b, continuity of flux gives the series resistance h/(2a) + h/(2b), hence kH = 2ab/(a + b), whereas kA = (a + b)/2. Their difference is (a - b)²/[2(a + b)]: it is O(h²) for smooth positive conductivity, but need not be small at a jump. At contrast c = 100, kA/kH = (c + 1)²/(4c) = 25.5025 on a mixed link. This is a local conductance ratio under the half-link interface assumption, not a global temperature-error estimate. Thus the arithmetic high-contrast results describe solver behaviour for that discrete operator; they do not validate composite-material heat flux. Harmonic averaging has the correct aligned series-resistance interpretation, while a curved interface requires additional geometric treatment. The matched-field audit includes both averages so that its conclusions are not restricted to arithmetic faces.', body)
    add_paragraph(
        doc,
    ...
    add_paragraph(
        doc,
        "The complete arithmetic/harmonic sensitivity results are reported below in Table 6.",
        body,
    )
    add_paragraph(
        doc,
    ...
    add_paragraph(doc, 'The complete face-averaging check in Table 6 reports all nine historical case-size cells. Arithmetic/harmonic iteration counts differ substantially, including AMG changes for checkerboard fields, although the setup-inclusive winner is unchanged in these single passes. Agreement of winners does not establish physical equivalence of the operators, timing stability, or identical solution accuracy. The aligned one-dimensional verification and the two-dimensional solver-cost sensitivity answer separate questions.', body)
    add_data_table(doc, 'Table 6. Complete contrast-100 face-averaging check under the historical monitored protocol. Entries are arithmetic/harmonic iteration counts and setup-inclusive winners (J = Jacobi-PCG, SA = AMG-PCG). These are single timing passes, not uncertainty estimates.', ['Case', 'N', 'CG', 'J', 'SA', 'Winner'], [list(r.values()) for r in read_csv_rows(ROOT/'results/analysis/protocol_sensitivity/face_table_rows.csv')])
    add_subsection(doc, "5.3", "Paired Callback Calibration")
    add_paragraph(doc, 'A prospective calibration covers constant, inclusion and checkerboard conductivity (contrast 100 for the latter two), N = 64, 128, 256, three methods, two callback modes and seven repeats: 378 recorded solves. Each repeat shuffles all 54 jobs; each solve constructs a fresh preconditioner with a seed matched across callback modes. Every run satisfies the independent residual threshold of 1e-8. All 189 paired solutions have identical byte hashes and iteration counts. Median paired monitored/count-only solve-time ratios range from 1.54 to 1.84 for CG, 1.58 to 1.77 for Jacobi-PCG, and 1.03 to 1.06 for AMG-PCG. These method-dependent penalties change reported speedups even when the median winner is unchanged.', body)
    add_paragraph(doc, 'Table 7 reports every case-size cell, including vote instability. The median winner agrees between callbacks in all nine cells. At constant N = 256, however, count-only AMG wins only three of seven repeats despite having the lowest method median; the median winner and majority vote answer different questions. The archived method summaries retain setup, solve and total-time quartiles for all candidates. Several N = 128 winners differ from the historical map under both callback modes, so callback removal alone cannot explain the historical-to-calibration difference. No universal N = 128 AMG crossover is inferred. The calibration does not measure cold-import overhead or control operating-system, power or thermal variation.', body)
    add_data_table(doc, 'Table 7. Paired callback calibration. Winner entries give method, median total milliseconds and votes out of seven. Last columns give median paired monitored/count-only solve-time ratios. J = Jacobi-PCG; SA = AMG-PCG.', ['Case', 'N', 'Monitored', 'Count-only', 'CG', 'J', 'SA'], [list(r.values()) for r in read_csv_rows(ROOT/'results/analysis/protocol_sensitivity/table_rows.csv')])
    add_subsection(doc, "5.4", "Matched-Field and Forcing Audit")
    ...
    add_paragraph(doc, 'For physical composite-conduction use, the face law must be chosen from the interface geometry and resistance model before comparing solvers. The arithmetic benchmark is not evidence of accurate cross-interface flux at high contrast. For performance use, the callback calibration shows that an iteration monitor can disproportionately penalise inexpensive iterations; solver rankings and their uncertainty must therefore be reported with the callback, setup and execution protocol.', body)
    add_paragraph(
        doc,
    ...
    add_paragraph(
        doc,
        'The previously published baseline and controlled extension are archived at https://doi.org/10.5281/zenodo.22668106, with the earlier baseline at https://doi.org/10.5281/zenodo.22303525. The present revision additionally supplies the callback calibration in results/raw/protocol_sensitivity, its prospective protocol in experiments/protocol_sensitivity.md, and complete generated summaries in results/analysis/protocol_sensitivity through the accompanying supplementary package. The complete revised artefact, including these calibration files, is archived at https://doi.org/10.5281/zenodo.22874266 (version v5). Raw histories are retained unchanged. Run experiments/analyze_protocol_sensitivity.py to regenerate the two new tables from the recorded CSV files. Recorded configurations and source hashes document the calibration. The reproduction instructions distinguish historical monitored runs from controlled count-only runs and preserve pilot failures. Tests check flux-energy identities, matched descriptors, reflection invariance and transpose equivalence. CPU-only tests explicitly skip unavailable CUDA tests; CUDA-required mode treats missing GPU support as an error. Timing reruns may differ with machine state.',
        body_first,
    )
    ...
    baseline = ROOT / 'tools' / 'review_baseline.docx'
    if baseline.exists():
        from highlight_manuscript_changes import highlight
        highlight(baseline, OUT, OUT, ROOT / 'paper' / 'REVISION_HIGHLIGHTS.json')
    print(OUT)
